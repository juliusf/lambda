...
{
    class SchemeBuiltinLessThanEquals : SchemeBuiltInFunction
    {
        public override SchemeObject evaluate(ref SchemeAST currentAST, SchemeEvaluator evaluator)
        {
            ...
            Debug.Assert(list[0].GetType() == typeof(SchemeBuiltinLessThanEquals));

            if (list.Count < 3)
            {
                throw new SchemeWrongNumberOfArguments(String.Format("Too few arguments! SchemeLessThanEquals need at least 2 arguments. You specified: {0}", list.Count - 1));
            }
            ...
            {
                throw new SchemeInvalidArgumentException("Builtin Function SchemeLessThanEquals expects SchemeInteger or SchemeFloat as parameter. Got something else.");
            }
